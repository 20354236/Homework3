...
from tools.tools import convert_to_square
import tools.vision as vision
from tools.tools import IoU
...
rnet_positive_file = '../DataSet/anno/pos_24.txt'
rnet_part_file = '../DataSet/anno/part_24.txt'
rnet_neg_file = '../DataSet/anno/neg_24.txt'
imglist_filename = '../DataSet/anno/imglist_anno_24.txt'

def gen_rnet_data(data_dir, anno_file, pnet_model_file, prefix_path='', device=torch.device("cpu")):
    """
    ...
    pnet, _, _ = create_mtcnn_net(p_model_path=pnet_model_file, device=device)
    mtcnn_detector = MtcnnDetector(pnet=pnet, min_face_size=12,device=device)
...
def gen_rnet_sample_data(data_dir, anno_file, det_boxs_file, prefix_path):
    ...
    f1 = open(rnet_positive_file, 'w')
    f2 = open(rnet_neg_file, 'w')
    f3 = open(rnet_part_file, 'w')

    # print(det_boxs_file)
    det_handle = open(det_boxs_file, 'rb')

    det_boxes = cPickle.load(det_handle)    #由Pnet预测出的候选框
    ...
    d_idx = 0
    image_done = 0
    for im_idx, dets, gts in zip(im_idx_list, det_boxes, gt_boxes_list):   #图像路径，Pnet选出的候选框， 真值

        gts = np.array(gts, dtype=np.float32).reshape(-1, 4)  #真值
        if gts.shape[0] == 0:
            continue
        if (image_done+1) % 10 == 0:
            print("%d images done" % (image_done+1))
        image_done += 1
...
if __name__ == '__main__':
    ...
    gen_rnet_data(traindata_store, annotation_file, pnet_model_file, prefix_path,device)
    print(1)
    ...
    anno_list.append(rnet_positive_file)
