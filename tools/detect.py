...
from tools.vision import vis_two

def create_mtcnn_net(p_model_path=None, r_model_path=None, o_model_path=None, device=torch.device("cpu")):
...
class MtcnnDetector(object):
    ...
    def detect_pnet(self, im):
        ...
        while min(current_height, current_width) > net_size:
            ...
            keep = tools.nms(boxes[:, :5], 0.5, 'Union')
            boxes = boxes[keep]
        ...
        keep = tools.nms(all_boxes[:, 0:5], 0.7, 'Union')
    ...
    def detect_onet(self, im, dets):
        ...
        keep = tools.nms(boxes, 0.7, mode="Minimum")
        ...
        bw = keep_boxes[:, 2] - keep_boxes[:, 0] + 1       #候选框
        bh = keep_boxes[:, 3] - keep_boxes[:, 1] + 1
    ...
    def detect_face(self,img):
        ...
        vis_two(cv2.merge([cv2.split(img)[2],cv2.split(img)[1],cv2.split(img)[0]]),boxes,boxes_align)
        ...
        vis_two(cv2.merge([cv2.split(img)[2],cv2.split(img)[1],cv2.split(img)[0]]),boxes,boxes_align,thresh=0.5)
